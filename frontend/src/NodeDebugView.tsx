...
function QueryLab({ status, unavailable }: { status: NativeSourceStatus | null; unavailable: boolean }) {
  const examples = queryExamples(status);
  const firstHeight = examples[0].request.height;
  const v2 = isArkivProfile(status);
  const types: EqSelection["valueType"][] = v2 ? ARKIV_TYPES : ["u64", "i64", "str", "bool"];
  const [selection, setSelection] = useState<EqSelection>({ ...examples[0].request });
  ...
  const run = async (draft: EqSelection, nextPage = 1) => {
    ...
    try {
      if (!types.includes(draft.valueType)) throw new Error("UnsupportedScalarType");
  ...
  useEffect(() => {
    if (!touched.current && !attempted && !unavailable && isReady(status, firstHeight)) {
      touched.current = true; void run(examples[0].request);
    }
  ...
  return <section className="nd-lab" aria-labelledby="query-title"><div className="nd-section-heading nd-lab-heading"><div><span className="nd-eyebrow">QUERY PLAYGROUND</span><h2 id="query-title">Don’t take the answer on trust.</h2><p>Pick a scenario, run a query, and follow the evidence.</p></div><span className="nd-chip"><Terminal size={13}/> Eq proof · {v2 ? "v2 / Arkiv entities" : "v1"}</span></div>
    <div className="nd-examples">{examples.map((sample) => <button type="button" key={sample.id} className={`nd-example ${example === sample.id ? "is-selected" : ""}`} aria-pressed={example === sample.id} onClick={() => {
...
function RecordRow({ row, entity }: { row: NativeRow; entity?: ArkivEntity }) {
  ...
  return <details className="nd-record"><summary><span className="nd-record-id">{displayNative(row.recordId).padStart(2, "0")}</span><span className="nd-record-main"><strong>{entity ? `${entity.key.slice(0, 12)}…${entity.key.slice(-8)}` : readableKey(row.recordKey)}</strong><span>{attributes.filter((a) => !a.name.startsWith("$")).slice(0, entity ? 2 : undefined).map((a) => `${a.name} = ${a.type}(${displayNative(a.value)})`).join(" · ")}</span></span><span className={`nd-expiry ${row.expiresAtHeight !== "18446744073709551615" ? "has-expiry" : ""}`}>{row.expiresAtHeight === "18446744073709551615" ? "Permanent" : `Expires #${displayNative(row.expiresAtHeight)}`}</span><ChevronRight size={15}/></summary><div className="nd-record-detail"><p><strong>Record key</strong> <code>{displayNative(row.recordKey)}</code></p>{fields.map((field) => <p key={field.name}><strong>{field.name}</strong> {field.type} · {formatNodeBytes(field.byteLength)} encoded</p>)}{entity && <div className="nd-system-fields"><h3>Verified entity system fields</h3>{Object.entries({"$owner":entity.owner,"$creator":entity.creator,"$createdAt":entity.createdAt,"$updatedAt":entity.updatedAt,"$expiresAt":entity.expiresAt,"$contentType":entity.contentType,"$creationFlags":json(entity.creationFlags)}).map(([name,value]) => <p key={name}><strong>{name}</strong><code>{value}</code></p>)}<RawData title="Verified payload bytes" value={entity.payload} name="entity-payload.hex" hex/></div>}<pre>{json(row)}</pre></div></details>;
}
