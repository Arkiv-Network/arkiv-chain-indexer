...
describe("parseServerConfig", () => {
  test("applies defaults", () => {
    ...
    expect(config).not.toHaveProperty("baseloadAutomationToken");
  ...
  test("reads from environment variables", () => {
    const config = parseServerConfig([], {
      ...
      BASELOAD_AUTOMATION_TOKEN: "env-secret",
...
describe("boolean flags from compose-style empty env values", () => {
  ...
  test("explicit values still apply", () => {
    ...
    expect(config).not.toHaveProperty("metricsBearerToken");
  });
});

test("legacy machine credentials are ignored", () => {
  const config=parseServerConfig([],{...BASE_ENV,BASELOAD_AUTOMATION_TOKEN:"old",METRICS_BEARER_TOKEN:"old"});
  expect(config).not.toHaveProperty("metricsBearerToken");
  expect(config).not.toHaveProperty("baseloadAutomationToken");
});
